...
/// Clock frequencies for Non-Secure TrustZone initialization.
///
/// The Secure world configures clocks before handing off to NS.
/// These values must match what the Secure world configured.
pub struct NsClockConfig {
    /// System clock frequency in Hz (typically 150_000_000)
    pub sys_hz: u32,
    /// Reference clock frequency in Hz (typically 12_000_000)
    pub ref_hz: u32,
    /// Peripheral clock frequency in Hz (typically = sys_hz)
    pub peri_hz: u32,
    /// USB clock frequency in Hz (must be 48_000_000)
    pub usb_hz: u32,
    /// ADC clock frequency in Hz (typically 48_000_000)
    pub adc_hz: u32,
    /// PLL_SYS frequency in Hz
    pub pll_sys_hz: u32,
    /// PLL_USB frequency in Hz
    pub pll_usb_hz: u32,
    /// XOSC frequency in Hz (typically 12_000_000)
    pub xosc_hz: u32,
}

impl NsClockConfig {
    /// Default RP2350 clock configuration (12 MHz crystal).
    /// Matches `ClockConfig::crystal(12_000_000)` and rp235x-hal's
    /// `init_clocks_and_plls(12_000_000, ...)` with default PLL settings.
    pub const fn rp2350_default() -> Self {
        Self {
            sys_hz: 150_000_000,
            ref_hz: 12_000_000,
            peri_hz: 150_000_000,
            usb_hz: 48_000_000,
            adc_hz: 48_000_000,
            pll_sys_hz: 150_000_000,
            pll_usb_hz: 48_000_000,
            xosc_hz: 12_000_000,
        }
    }
}

/// Initialize Embassy for Non-Secure TrustZone operation.
///
/// Use this instead of [`init()`] when the Secure world has already configured
/// clocks, PLLs, and tick generators. This function:
/// - Steals peripheral singletons (no singleton check — Secure world didn't call `take()`)
/// - Populates the internal clock frequency bookkeeping
/// - Initializes the time driver, DMA, and GPIO interrupts
/// - Does NOT touch RESETS, PLLs, XOSC, or clock muxes
///
/// # Safety
/// - Clocks must already be configured by the Secure world
/// - The provided frequencies must match the actual hardware configuration
/// - Must only be called once
pub unsafe fn init_ns(clocks: NsClockConfig) -> Peripherals {
    let peripherals = Peripherals::steal();

    // Populate the CLOCKS static so Embassy drivers know the frequencies
    clocks::set_frequencies(
        clocks.xosc_hz,
        clocks.sys_hz,
        clocks.ref_hz,
        clocks.pll_sys_hz,
    ...
    // Initialize subsystems that just enable interrupts (no hardware reset)
    #[cfg(feature = "time-driver")]
    time_driver::init();
    dma::init();
    gpio::init();
...
#[cfg(all(feature = "rt", not(feature = "trustzone-ns")))]
