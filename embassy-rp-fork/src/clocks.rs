...
/// Set clock frequencies for Non-Secure TrustZone init.
///
/// Called by `init_ns()` to populate the CLOCKS static without touching hardware.
/// The Secure world must have already configured the actual clock tree.
pub(crate) fn set_frequencies(
    xosc_hz: u32,
    sys_hz: u32,
    ref_hz: u32,
    pll_sys_hz: u32,
    ...
    CLOCKS.xosc.store(xosc_hz, Ordering::Relaxed);
    CLOCKS.sys.store(sys_hz, Ordering::Relaxed);
    CLOCKS.reference.store(ref_hz, Ordering::Relaxed);
    CLOCKS.pll_sys.store(pll_sys_hz, Ordering::Relaxed);
    CLOCKS.pll_usb.store(pll_usb_hz, Ordering::Relaxed);
    CLOCKS.usb.store(usb_hz, Ordering::Relaxed);
    CLOCKS.adc.store(adc_hz, Ordering::Relaxed);
    CLOCKS.peri.store(peri_hz, Ordering::Relaxed);
    CLOCKS.rosc.store(0, Ordering::Relaxed);
}
