...
class BitcoinService {
  ...
  Future<(String, BigInt)> broadcastTransaction(
      String deviceId, String txHex, PolicyState policyState) async {
    ...
    try {
      if (policyState.normalPolicy == null) {
        print(
            '[$deviceId] Warning: No normal policy found, cannot detect change UTXOs.');
      } else {
        ...
        final existingJson = _utxoStore.getUtxo(deviceId);
        ...
        if (detectedChange.isNotEmpty || inputsRemoved > 0) {
          ...
          await _utxoStore.saveUtxo(deviceId, jsonEncode(currentList));
          print(
              '[$deviceId] Updated UTXOs: Removed $inputsRemoved inputs, Added ${detectedChange.length} change. Net Spent: $spentAmount sats.');
        }
      }
    } catch (e) {
      print('[$deviceId] Error processing change outputs: $e');
